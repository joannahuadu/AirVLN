...
sys.path.append("/mnt/data1/workspace/wmq/AirVLN_ws/AirVLN")
from Model import *
...
class IWTrajectoryDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.index = []
        kept, skipped = 0, 0
        for ep in self.list_data_dict.get("episodes", []):
            ep_id = ep["episode_id"]
            if ep_id in self.keys:
                self.episodes_by_id[ep_id] = ep
                n_frames = len(ep["reference_path"])
                for fidx in range(1, n_frames + 1):
                    self.index.append((ep_id, fidx))
                kept += 1
            else:
                skipped += 1
    ...
    def _get_episode_record(self, ep_id):
        with lmdb.open(
            self.lmdb_features_dir,
            map_size=int(self.lmdb_map_size),
            readonly=True,
            ...
            raw = txn.get(ep_id.encode())
            if raw is None:
                raise KeyError(f"LMDB missing key: {ep_id}")
            return msgpack_numpy.unpackb(raw, raw=False)
    ...
    def __getitem__(self, idx: int):
        ep_id, frame_num = self.index[idx]
        ep = self.episodes_by_id[ep_id]
        record = self._get_episode_record(ep_id)

        rgb = record[0]['rgb'][frame_num - 1]
        depth = record[0]['depth'][frame_num-1]
        assert len(ep['actions']) == len(ep['reference_path'])
        try:
            assert np.all(ep['actions'] == record[2])
        except:
            print("np.all(source['actions'] == new[2]): ", np.sum(ep['actions'] == record[2]), ep_id)
        src = {
            'episode_id': ep_id,
            'rgb': rgb,
            'depth': depth,
            'conversations': ep['instruction']['instruction_text'],
            'length': len(ep['reference_path']),
            'actions': ep['actions'],
            'reference_path': ep['reference_path'],
            'frame': frame_num,
        }
        
        ori_sources = copy.deepcopy(src)
        image = src['rgb']
        ...
        stage, future_delta, assist = self.get_stage(src['reference_path'], frame_num)
        cur_pos = src['reference_path'][frame_num - 1][:3]
        ...
        sources = preprocess_multimodal(
            copy.deepcopy([e["conversations"] for e in [src]]),
        ...
        if len(waypoint) == 0:
            waypoint = np.array([history_waypoint[-1] for _ in range(7)])
        elif len(waypoint) < 7:
